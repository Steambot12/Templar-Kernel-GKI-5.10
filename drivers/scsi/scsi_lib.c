...
static void scsi_dec_host_busy(struct Scsi_Host *shost, struct scsi_cmnd *cmd)
{
	...
	if (unlikely(scsi_host_in_recovery(shost))) {
		 ...
		 * other CPUs before scsi_eh_wakeup() checks busy requests.
		 * Otherwise, reordering can cause CPUs to race and miss an
		 * eh wakeup when no CPU sees all busy requests as done or
		 * timed out.
		 */
		smp_mb();
